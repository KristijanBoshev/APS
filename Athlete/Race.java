...
public class Race {
    ...
    public static void competition(SLL<Athlete> prevWin, SLL<Athlete> currWin) {
        SLLNode<Athlete> curr = prevWin.getFirst();
        double worstTime = 0;

        while(curr != null){
            if(worstTime < curr.element.getTime()){
                worstTime = curr.element.getTime();
            }
            curr = curr.succ;
        }
        curr = currWin.getFirst();
        while(curr != null){
            if(curr.element.getTime() > worstTime){
                currWin.delete(curr);
            }
            curr = curr.succ;
        }
