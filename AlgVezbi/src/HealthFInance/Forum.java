...
public class Forum {
    ...
    public static void forum(SLL<Discussion> health, SLL<Discussion> finance) {
        SLLNode<Discussion> curr = health.getFirst();
        SLLNode<Discussion> other = finance.getFirst();
        SLLNode<Discussion> pom = null;
        int min = 9999;

        while(other != null){
            int imp = other.element.getPopularity() * other.element.getUsers();
            if(min>imp){
                min = imp;
                pom = other;
            }
        ...
        while(curr != null){
            int imp = curr.element.getPopularity() * curr.element.getUsers();
            if(max<imp){
                max = imp;
                pom = curr;
            }
        ...
        finance.insertLast(pom.element);
